...
class SFTDataset(Dataset):
    def __init__(self,tokeinzer,context_len,input_file):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        for chance in data:
            ...
            content_ids = self.tokenizer.encode(content,add_special_tokens=False).input_ids
        ...
        inputs = torch.tensor(tokens[:-1],type=torch.long)
        targets = torch.tensor(labels[1:],type=torch.long)          
